 ...
 * 举报
 *
 * @property-read \App\Model\Report      currentModel
 *
 * Class ReportService
 * @package App\Service
 */
class Report extends ServiceAbstracts
{
    use Base;
    ...
    public function getList(bool $withRelationship = false): array
    {
        ...
        $list = $this->container->reportModel
            ->where($where)
            ->field([
    ...
    public function create(string $reportableType, int $reportableId, string $reason): int
    {
        ...
        return (int)$this->container->reportModel->insert([
            'reportable_type' => $reportableType,
            'reportable_id'   => $reportableId,
            'user_id'         => $userId,
            'reason'          => $reason,
    ...
    private function createValidator(string $reportableType, int $reportableId, string $reason): void
    {
        ...
        // 验证是否已举报过
        $isExist = $this->container->reportModel
            ->where([
                'reportable_type' => $reportableType,
                'reportable_id' => $reportableId,
                'user_id' => $this->container->roleService->userId(),
            ])
            ->has();
    ...
    public function deleteMultiple(array $targets): void
    {
        ...
        if ($where) {
            $this->container->reportModel->where($where)->delete();
        }
    }
