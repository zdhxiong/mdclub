 ...
 * 用户头像
 */
class Avatar extends Abstracts
{
    use Brandable;
    ...
    public function upload(int $userId, UploadedFileInterface $avatar = null): string
    {
        ...
        $userInfo = $this->model->field(['user_id', 'avatar'])->get($userId);
        $this->deleteImage($userId, $userInfo['avatar']);

        $filename = $this->uploadImage($userId, $avatar);
        $this->model->where('user_id', $userId)->update('avatar', $filename);
    ...
    {
        $userInfo = $this->model->field(['user_id', 'username', 'avatar'])->get($userId);
        if (!$userInfo) {
