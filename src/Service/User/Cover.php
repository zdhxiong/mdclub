...
class Cover extends Abstracts
{
    ...
    public function upload(int $userId, UploadedFileInterface $cover = null): string
    {
        ...
        $userInfo = $this->model->field(['user_id', 'cover'])->get($userId);
        if ($userInfo['cover']) {
            $this->deleteImage($userId, $userInfo['cover']);
        }

        $filename = $this->uploadImage($userId, $cover);
        $this->model->where('user_id', $userId)->update('cover', $filename);
    ...
    public function delete(int $userId): void
    {
        ...
        if ($userInfo['cover']) {
            $this->deleteImage($userId, $userInfo['cover']);
            $this->model->where('user_id', $userId)->update('cover', '');
        }
    }
}
