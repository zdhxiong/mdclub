 ...
 * 话题
 *
 * @property-read \App\Model\Topic      currentModel
 *
 * Class Topic
 * @package App\Service
 */
class Topic extends ServiceAbstracts
{
    use Base, Followable, Brandable;
    ...
    public function getList(array $condition = [], bool $withRelationship = false): array
    {
        ...
        if (isset($condition['is_deleted']) && $condition['is_deleted']) {
            $this->container->topicModel->onlyTrashed();
        ...
        $list = $this->container->topicModel
            ->where($where)
            ->order($order)
    ...
    public function create(
        ...
        // 添加话题
        $topicId = (int)$this->container->topicModel->insert([
            'name' => $name,
    ...
    private function createValidator(
        ...
        if (!$errors && $this->container->topicModel->where(['name' => $name])->has()) {
            $errors['name'] = '该名称已存在';
        }
    ...
    private function updateValidator(
        ...
        $topicInfo = $this->container->topicModel->get($topicId);
        if (!$topicInfo) {
        ...
        if (
               !$errors
            && !is_null($name)
            && $this->container->topicModel->where(['name' => $name, 'topic_id[!]' => $topicId])->has()
        ) {
            $errors['name'] = '该名称已存在';
        }
    ...
    {
        $rowCount = $this->container->topicModel->delete($topicId);
        ...
        // 删除话题关系
        $this->container->topicableModel->where(['topic_id' => $topicId])->delete();
    ...
    public function deleteMultiple(array $topicIds): void
    {
        ...
        $topics = $this->container->topicModel
            ->field(['topic_id'])
            ->select($topicIds);
        ...
        $this->container->topicModel->delete($topicIds);
        ...
        // 删除话题关系
        $this->container->topicableModel->where(['topic_id' => $topicIds])->delete();
    ...
    public function getInRelationship(array $targetIds, string $targetType): array
    {
        ...
        $topicsTmp = $this->container->topicModel
            ->join([
        ...
        foreach ($topicsTmp as $item) {
            $topics[$item['topicable_id']][] = $this->container->topicService->handle([
    ...
    public function addRelationship(array $topics, array $relationship = []): array
    {
        ...
        if (isset($relationship['is_following'])) {
            ...
            $followingTopicIds = $this->container->followService->getIsFollowingInRelationship($topicIds, 'topic');
        }
