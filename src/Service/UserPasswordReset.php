 ...
 * @package App\Service
 */
class UserPasswordReset extends ServiceAbstracts
{
    /**
     * 密码重置时，发送邮箱验证邮件
     *
    ...
    public function sendEmail(string $email): void
    {
        ...
        $this->container->emailService->sendPasswordResetEmail($email);
    }
    ...
    public function doReset(string $email, string $emailCode, string $password): void
    {
        ...
        $this->container->tokenModel->where(['user_id' => $userInfo['user_id']])->delete();
    }
    ...
    private function sendEmailValidator(string $email, bool $needCaptcha): void
    {
        ...
        if (!$errors) {
            $userInfo = $this->container->userModel->where(['email' => $email])->get();
