 ...
 * 用户，已禁用用户也要当普通用户处理
 *
 * @property-read \App\Model\User      currentModel
 *
 * Class User
 * @package App\Service
 */
class User extends ServiceAbstracts
{
    use Base, Followable;
    ...
    public function has(int $userId): bool
    {
        ...
        if ($userId == $this->container->roleService->userId()) {
            return true;
        }

        return $this->container->userModel->has($userId);
    }
    ...
    public function hasMultiple(array $userIds): array
    {
        ...
        $existUserIds = $this->container->userModel
    ...
    public function get(int $userId, bool $withRelationship = false): array
    {
        ...
        if ($this->container->roleService->userId() != $userId) {
            $excludeFields = ArrayHelper::push($excludeFields, $this->getPrivacyFields());
        }

        $userInfo = $this->container->userModel
    ...
    public function update(int $userId, array $data): void
    {
        ...
        $this->container->userModel->where(['user_id' => $userId])->update($data);
    }
    ...
    {
        $requestTime = $this->container->request->getServerParams()['REQUEST_TIME'];

        $rowCount = $this->container->userModel
        ...
        // 禁用后，删除该用户的所有token
        $this->container->tokenModel->where(['user_id' => $userId])->delete();
    }
    ...
    public function disableMultiple(array $userIds): void
    {
        ...
        if ($rowCount) {
            $this->container->tokenModel->where(['user_id' => $userIds])->delete();
        }
    }
    ...
    public function handle(array $users): array
    {
        ...
        foreach ($users as &$user) {
            if (isset($user['avatar'])) {
                $user['avatar'] = $this->container->userAvatarService->getBrandUrls($user['user_id'], $user['avatar']);
            }

            if (isset($user['cover'])) {
                $user['cover'] = $this->container->userCoverService->getBrandUrls($user['user_id']);
            }
        }
    ...
    {
        return $this->container->userModel->where(['email' => $email])->has();
    }
    ...
    {
        return $this->container->userModel->where(['username' => $username])->has();
    }
    ...
    {
        $users = $this->container->userModel
    ...
    public function getInRelationship(array $userIds): array
    {
        ...
        $usersTmp = $this->container->userModel
            ->where(['user_id' => $userIds])
            ->field(['user_id', 'avatar', 'username', 'headline'])
            ->select();

        $usersTmp = $this->container->userService->handle($usersTmp);
    ...
    public function addRelationship(array $users, array $relationship = []): array
    {
        ...
        if ($userIds && $currentUserId && ($isArray || $currentUserId != $users[0]['user_id'])) {
            if (isset($relationship['is_followed'])) {
                ...
                $followedUserIds = $this->container->followModel->where([
                    'user_id' => $userIds,
            ...
            if (isset($relationship['is_following'])) {
                ...
                $followingUserIds = $this->container->followService->getIsFollowingInRelationship($userIds, 'user');
            }
        }
