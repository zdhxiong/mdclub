 ...
 */
namespace MDClub\Vendor;

/**
 * Class MDAvatars
 *
 * @package MDClub\Vendor
 */
class MDAvatars
{
    public $Char;
    ...
    /**
     * MDAvatars constructor.
     *
     * @param     $Char
     * @param int $AvatarSize
     */
    public function __construct($Char, $AvatarSize = 256)
    {
        $this->Char            = mb_strtoupper(mb_substr($Char, 0, 1, "UTF-8"));
        $this->AvatarSize      = $AvatarSize;
        $this->Padding         = 30 * ($this->AvatarSize / 256);
        $this->LetterFont      = dirname(__FILE__) . '/MDAvatars-fonts/Roboto-Light.ttf';
        $this->AsianFont       = dirname(__FILE__) . '/MDAvatars-fonts/NotoSansSC-Regular.otf';
        $this->EnableAsianChar = is_file($this->AsianFont);

        $CNChar = ord($this->Char);
        if (
            !$this->EnableAsianChar &&
            ...
            if ($Code >= -20319 and $Code <= -20284) {
                $this->Char = "A";
            }
            if ($Code >= -20283 and $Code <= -19776) {
                $this->Char = "B";
            }
            if ($Code >= -19775 and $Code <= -19219) {
                $this->Char = "C";
            }
            if ($Code >= -19218 and $Code <= -18711) {
                $this->Char = "D";
            }
            if ($Code >= -18710 and $Code <= -18527) {
                $this->Char = "E";
            }
            if ($Code >= -18526 and $Code <= -18240) {
                $this->Char = "F";
            }
            if ($Code >= -18239 and $Code <= -17923) {
                $this->Char = "G";
            }
            if ($Code >= -17922 and $Code <= -17418) {
                $this->Char = "H";
            }
            if ($Code >= -17417 and $Code <= -16475) {
                $this->Char = "J";
            }
            if ($Code >= -16474 and $Code <= -16213) {
                $this->Char = "K";
            }
            if ($Code >= -16212 and $Code <= -15641) {
                $this->Char = "L";
            }
            if ($Code >= -15640 and $Code <= -15166) {
                $this->Char = "M";
            }
            if ($Code >= -15165 and $Code <= -14923) {
                $this->Char = "N";
            }
            if ($Code >= -14922 and $Code <= -14915) {
                $this->Char = "O";
            }
            if ($Code >= -14914 and $Code <= -14631) {
                $this->Char = "P";
            }
            if ($Code >= -14630 and $Code <= -14150) {
                $this->Char = "Q";
            }
            if ($Code >= -14149 and $Code <= -14091) {
                $this->Char = "R";
            }
            if ($Code >= -14090 and $Code <= -13319) {
                $this->Char = "S";
            }
            if ($Code >= -13318 and $Code <= -12839) {
                $this->Char = "T";
            }
            if ($Code >= -12838 and $Code <= -12557) {
                $this->Char = "W";
            }
            if ($Code >= -12556 and $Code <= -11848) {
                $this->Char = "X";
            }
            if ($Code >= -11847 and $Code <= -11056) {
                $this->Char = "Y";
            }
            if ($Code >= -11055 and $Code <= -10247) {
                $this->Char = "Z";
            }
        }
    ...
    private function initialize()
    {
        ...
        if ($this->IsNotLetter) {
            ...
            $Y        = $Height - $Padding - (6 / 196) * $FontSize;
        }
        // 在圆正中央填入字符
        imagettftext(
            $this->Avatar,
